...
const Feedback = (props) => {
    ...
    const onSubmit = async (data, e) => {
        ...
        if (res === "notadded") {
            setError("exist", { message: "Invaid Input" })
        }
        else {
            alert("Your Feedback has Saved in our Database and will be Review by Devloper")
        }
    }
    ...
    if (props.user.name == "" && props.user.email == "") {
        ...
        name = <input value={props.user.name} {...register("name", { required: { value: true, message: "Enter the Name" } })} className="w-[70%] px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white" type="text" placeholder="Name" id='name' name='name' />
        email = <input value={props.user.email} {...register("email", { required: { value: true, message: "Enter the Email" } })} className="w-[70%] px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-5" type="email" placeholder="Email" id='email' name='email' />
    }
    return (
        <div>
            <section className="text-gray-600 body-font relative">
                <div className="container px-5 py-6 mx-auto flex sm:flex-nowrap flex-wrap">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className='h-[70vh] flex justify-center items-center flex-col w-1/3' ref={ref} >
                        ...
                        <input type="Submit" value="Register a Account" className="mt-5 tracking-wide font-semibold bg-[#47B884] text-white-500 w-[70%] py-4 rounded-lg hover:bg-green-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none" />
