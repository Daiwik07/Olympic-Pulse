...
const Calendar = () => {
    ...
    // Olympic and Paralympic events (simplified for example)
    const olympicEvents = [
    ...
    useEffect(() => {
        // Prepopulate the calendar with Olympic and Paralympic events
    ...
    const handleAddEvent = () => {
        const event = prompt(`Add event for ${currentMonth.name} ${selectedDay}`);
        if (event && selectedDay) {
            setEvents([...events, { day: selectedDay, event, month: currentMonth.name }]);
        }
    };
